            ...
            while (true)
            {
                try
                {
                    ...
                    if (schemaGuid != Guid.Empty)
                    {
                        ...
                        ret.Add(schemaGuid, new PowerPlanInfo() { SchemeGuid = schemaGuid, FriendlyName = schemaFriendlyName });
                    }
                    else
                        break;
        ...
        private class PowrProfHelper
        {
            ...
            public static Guid GetPowerSchemeGuid(UInt32 index)
            {
                ...
                try
                {
                    ...
                    {
                        Debug.Write(string.Format("PowerSchemeSwitcher: PowerEnumerate return {0} for index: {1}", res, index));
